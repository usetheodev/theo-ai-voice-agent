...
class RTPServer:
    ...
    def __init__(self,
        ...
        """
        Initialize RTP server with dynamic port allocation.

        Args:
            host: Host to bind to
            port: Base port (used if port_range not specified)
            config: Configuration dictionary
            allowed_asterisk_ips: Whitelist of Asterisk IPs (security)
                                 Default: ['127.0.0.1', '::1', '172.20.0.10']
            port_range: Tuple (start, end) for dynamic port allocation per call
                       Default: (18000, 18100) - 100 ports for concurrent calls
        """
        self.host = host
        self.port = port  # Base port (legacy, kept for compatibility)
        self.config = config or {}
        self.logger = logging.getLogger("ai-voice-agent.rtp")

        # Port allocation for per-call sockets
        if port_range is None:
            port_range = self.config.get('rtp_port_range', (18000, 18100))
        self.port_range = port_range
        self.port_allocation: Dict[int, str] = {}  # port -> call_id mapping
        self.logger.info(f"🔌 Dynamic port range: {self.port_range[0]}-{self.port_range[1]}")
        ...
        # REMOVED: Main UDP socket (now per-call sockets)
        self.sock = None
    ...
    def _reserve_port(self, call_id: str) -> Optional[int]:
        """
        Reserve a dynamic RTP port for a call.
        ...
        for port in range(start, end + 1):
            if port not in self.port_allocation:
                ...
                return port
        self.logger.error(f"❌ No available RTP ports in range {start}-{end}")
        return None

    def _release_port(self, port: int):
        """Release a dynamically allocated RTP port."""
        if port in self.port_allocation:
            del self.port_allocation[port]
            self.logger.debug(f"🔓 Released RTP port {port}")

    async def allocate_port(self, call_id: str) -> Optional[int]:
        """
        Public method to allocate a port for a call.
        Must be called BEFORE creating ExternalMedia channel.

        Returns:
            Allocated port or None on failure
        """
        port = self._reserve_port(call_id)
        if port:
            self.logger.info(f"🔌 Allocated RTP port {port} for call {call_id}")
        return port

    async def create_session_with_port(self, call_id: str, port: int) -> Optional[CallSession]:
        """
        Create RTP session with dedicated socket on allocated port.
        Called AFTER port allocation, BEFORE ExternalMedia channel creation.

        Args:
            call_id: Unique call identifier
            port: Allocated RTP port

        Returns:
            CallSession with dedicated socket or None on failure
        """
        try:
            # Create dedicated UDP socket for this call
            ...
            self.logger.info(f"📞 New session created: {call_id} on port {port}")

            # Create session (remote_addr will be set when first packet arrives)
            session = CallSession(
                call_id=call_id,
                socket=sock,
                remote_addr=None  # Will be set by symmetric RTP
            )
            ...
            # RTPBuilder will be initialized when we know outbound SSRC
            # (after first packet arrives and symmetric RTP is established)
            session.rtp_builder = None

            # Store session
            self.sessions[call_id] = session

            # Start per-call RTP receiver task
            asyncio.create_task(self._rtp_receiver_task(session, port))

            self.logger.info(f"✅ Session ready: {call_id} listening on {self.host}:{port}")
            return session

        except Exception as e:
            self.logger.error(f"Failed to create session for {call_id}: {e}", exc_info=True)
            if 'sock' in locals():
                sock.close()
    ...
    async def start(self):
        """Start the RTP server (no shared socket - per-call sockets created dynamically)."""
        try:
            self.logger.info(f"✅ RTP Server initialized (dynamic port mode: {self.port_range[0]}-{self.port_range[1]})")
    ...
    async def stop(self):
        ...
        # Note: Per-call sockets are closed in cleanup_session()
        # No shared socket to close anymore

        self.logger.info("✅ RTP Server stopped")

    async def _rtp_receiver_task(self, session: CallSession, port: int):
        """Per-call RTP receiver task (dedicated socket per call)."""
        loop = asyncio.get_event_loop()
        call_id = session.call_id
        sock = session.socket

        self.logger.info(f"📡 RTP receiver started for {call_id} on port {port}")

        try:
            self.logger.info(f"📡 Receiver loop starting for {call_id}")
            while self._running and call_id in self.sessions:
                # Receive packet (non-blocking)
                try:
                    data, addr = await loop.sock_recvfrom(sock, 2048)
                    self.logger.info(f"📦 Received {len(data)} bytes from {addr[0]}:{addr[1]}")
                except Exception as e:
                    self.logger.error(f"❌ sock_recvfrom error: {e}", exc_info=True)
                ...
                # Set remote address from first packet if not already set (symmetric RTP fallback)
                if session.remote_addr is None:
                    session.remote_addr = addr
                    self.logger.info(f"🔗 Remote endpoint learned from first packet: {addr[0]}:{addr[1]}")

                # Initialize SSRC and RTPBuilder on first packet (regardless of remote_addr)
                if session.inbound_ssrc is None:
                    session.inbound_ssrc = ssrc
                    session.generate_outbound_ssrc()

                    # Initialize RTPBuilder with outbound SSRC
                    session.rtp_builder = RTPBuilder(
                        ssrc=session.outbound_ssrc,
                        initial_sequence=0,
                        initial_timestamp=0
                    )

                    self.logger.info(f"🔗 SSRC established for {call_id}: from {addr[0]}:{addr[1]}")
                    self.logger.info(f"   Inbound SSRC:  {session.inbound_ssrc:#010x}")
                    self.logger.info(f"   Outbound SSRC: {session.outbound_ssrc:#010x}")
                    self.logger.info(f"   RTP destination: {session.remote_addr[0]}:{session.remote_addr[1]}")

                # DEBUG: Log RTP packets
            ...
            self.logger.info(f"📡 RTP receiver stopped for {call_id}")
        except Exception as e:
            self.logger.error(f"Error in RTP receiver for {call_id}: {e}", exc_info=True)
    ...
    async def _send_silence_keepalive(self, session: CallSession):
        ...
        try:
            ...
            await asyncio.get_event_loop().sock_sendto(
                session.socket,  # Use per-call socket
                rtp_packet,
                session.remote_addr
            )
        ...
        try:
            ...
            for i in range(num_frames):
                ...
                try:
                    ...
                    if packets_sent == 0:
                        self.logger.info(f"📍 First RTP packet sent: to={session.remote_addr}, len={len(rtp_packet)}, bytes_sent={bytes_sent}, socket_bound={session.socket.getsockname()}")
                        self.logger.info(f"   Socket info: fd={session.socket.fileno()}, family={session.socket.family}, type={session.socket.type}")
                except Exception as e:
    ...
    async def cleanup_session(self, call_id: str):
        ...
        # =========================================================
        # Phase 5.2: Cleanup Per-Call Socket and Release Port
        # =========================================================

        # Close per-call dedicated socket
        if session.socket:
            try:
                # Get port before closing socket
                port = session.socket.getsockname()[1]
                session.socket.close()
                self.logger.info(f"🔌 Closed dedicated socket on port {port} for {call_id}")

                # Release port back to pool
                self._release_port(port)
                self.logger.info(f"🔓 Released port {port} for {call_id}")
            except Exception as e:
                self.logger.error(f"❌ Error closing socket/releasing port for {call_id}: {e}", exc_info=True)
