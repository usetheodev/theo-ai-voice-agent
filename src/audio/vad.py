...
- WebRTC VAD: ML-based, robust to noise (300k downloads/month, ZERO CVEs)
...
Pattern based on:
- Asterisk-AI-Voice-Agent (EnhancedVADManager, line 111-119)
- Pipecat AI (dual-mode VAD for voice agents)

Evidence:
- webrtcvad 2.0.10: 300k downloads/month, ZERO CVEs (NVD database)
- Sample rate 8000 Hz supported (official docs)
- Aggressiveness 1 (balanced) - validated in Asterisk-AI
"""
...
class VoiceActivityDetector:
    ...
    Confidence Scoring:
        - WebRTC agrees with Energy: confidence += 20%
        - WebRTC detects speech: confidence += 40%
        - Energy detects speech: confidence += 40%
        - Total: 0.0 - 1.0 scale

    Pattern: Asterisk-AI-Voice-Agent (EnhancedVADManager line 89-180)
    """
    ...
    def __init__(self,
                 ...
                 webrtc_aggressiveness: int = 1,  # NEW: 0-3 (1=balanced)
        ...
        if WEBRTC_VAD_AVAILABLE:
            try:
                ...
                if sample_rate not in [8000, 16000, 32000]:
                    self.logger.warning(
                        f"WebRTC VAD requires 8k/16k/32k Hz, got {sample_rate} Hz. "
                        f"Using energy-only mode."
                    )
                    ...
                    self.logger.info(
                        f"✅ WebRTC VAD initialized (aggressiveness={webrtc_aggressiveness}, "
                        f"sample_rate={sample_rate} Hz)"
                    )
            except Exception as e:
                self.logger.warning(f"WebRTC VAD init failed: {e}. Using energy-only mode.")
                self.webrtc_vad = None
                self.webrtc_mode = "energy-only"
        else:
            self.logger.info("WebRTC VAD not available (pip install webrtcvad). Using energy-only mode.")
            self.webrtc_mode = "energy-only"
        ...
        self.webrtc_detections = 0  # NEW
        self.energy_detections = 0  # NEW
        self.agreement_count = 0    # NEW (WebRTC agrees with Energy)

        self.logger.info(
            f"VAD initialized ({self.webrtc_mode}): "
    ...
    def process_frame(self, pcm_data: bytes) -> bool:
        ...
        This means:
            ...
            - Pattern: Asterisk-AI-Voice-Agent line 148-168
        ...
        if self.webrtc_vad:
            try:
                ...
                self.logger.debug(f"WebRTC VAD error: {e}")
                webrtc_result = False
        ...
        if self.state == VADState.SILENCE:
            ...
            if speech_continues:
                ...
                self.logger.debug(
                    f"Speech pending end (webrtc={webrtc_result}, energy={energy:.1f} < {self.energy_threshold_end:.1f})"
                )
                return True
    ...
    def get_stats(self) -> dict:
        ...
        stats = {
            ...
            'mode': self.webrtc_mode,  # NEW: dual-mode / energy-only
