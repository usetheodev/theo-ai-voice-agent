...
class G711Codec:
    ...
    def __init__(self, law: str = 'ulaw'):
        ...
        self.law = law
        self.logger = logging.getLogger(f"ai-voice-agent.codec.g711.{law}")
        ...
        self.logger.info(f"G.711 {law.upper()} codec initialized")
    ...
    def encode(self, pcm_data: bytes) -> Optional[bytes]:
        ...
        try:
            ...
            self.logger.error(f"Encode error: {e}", exc_info=True)
            return None
    ...
    def decode_to_numpy(self, data: bytes, dtype=np.int16) -> Optional[np.ndarray]:
        ...
        try:
            ...
            self.logger.error(f"Numpy conversion error: {e}", exc_info=True)
            return None
    ...
    def encode_from_numpy(self, samples: np.ndarray) -> Optional[bytes]:
        ...
        try:
            ...
            self.logger.error(f"Numpy encode error: {e}", exc_info=True)
            return None
